...
选项:
    ...
    --theme, -t          排版主题：default, playful-geometric, neo-brutalism,
                         botanical, professional, retro, terminal, sketch, cinematic
    --mode, -m           分页模式：
...
def generate_cover_html(metadata: dict, theme: str, width: int, height: int) -> str:
    ...
    theme_backgrounds = {
        ...
        'cinematic': 'linear-gradient(180deg, #1D2325 0%, #2D3538 100%)'
    }
    ...
    title_gradients = {
        ...
        'cinematic': 'linear-gradient(180deg, #BA8D5F 0%, #F0E6D3 100%)',
    }
...
def generate_card_html(content: str, theme: str, page_number: int = 1, 
    ...
    theme_backgrounds = {
        ...
        'cinematic': 'linear-gradient(135deg, #1D2325 0%, #2D3538 100%)'
    }
    bg = theme_backgrounds.get(theme, theme_backgrounds['default'])
